...
description: >-
  Convert 'Applied intelligently' Agent rules (.agents/rules/*.md) and slash
  commands (.agents/commands/*.md) to Agent Skills format (.agents/skills/). Use
  when you want to migrate rules or commands to skills, convert .md rules to
...
Convert Agent rules ("Applied intelligently") and slash commands to Agent Skills format.
...
|-------|--------|-------------|
| Project | `{workspaceFolder}/**/.agents/rules/*.md`, `{workspaceFolder}/.agents/commands/*.md` |
| User | `~/.agents/commands/*.md` |

Notes:
- Agent rules inside the project can live in nested directories. Be thorough in your search and use glob patterns to find them.
...
```markdown
# Before: .agents/rules/my-rule.md
---
...
```markdown
# Before: .agents/commands/commit.md
# Commit current work
Instructions here...
```
...
4. Create the skill directory: `.agents/skills/{skill-name}/` (skill name = filename without .md)
...
1. [ ] Create the skills directories if they don't exist (`.agents/skills/` for project, `~/.agents/skills/` for user)
2. Dispatch three fast general purpose subagents (NOT explore) in parallel to do the following steps for project rules (pattern: `{workspaceFolder}/**/.agents/rules/*.md`), user commands (pattern: `~/.agents/commands/*.md`), and project commands (pattern: `{workspaceFolder}/**/.agents/commands/*.md`):
...
1. [ ] Create the skills directories if they don't exist (`.agents/skills/` for project, `~/.agents/skills/` for user)
2. [ ] Find files to migrate in both project (`.agents/`) and user (`~/.agents/`) directories
